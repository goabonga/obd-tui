...
class TestEngine:
    def test_shows_the_readings_the_vehicle_answered(self) -> None:
        ...
        text = engine.render(state, EMPTY)
    ...
    def test_hides_a_section_the_vehicle_did_not_answer(self) -> None:
        text = engine.render(VehicleState(rpm=1450.0), EMPTY)
    ...
    def test_reports_when_nothing_was_read(self) -> None:
        assert engine.render(VehicleState(), EMPTY) == NO_DATA

    def test_run_time_is_a_duration(self) -> None:
        assert "01:01:01" in engine.render(VehicleState(run_time=3661), EMPTY)
...
class TestAir:
    ...
    def test_shows_the_throttle_section(self) -> None:
        assert "THROTTLE" in air.render(VehicleState(throttle=12.0), EMPTY)

    def test_reports_when_nothing_was_read(self) -> None:
        assert air.render(VehicleState(), EMPTY) == NO_DATA


class TestEgr:
    def test_explains_a_valve_below_the_command(self) -> None:
        text = egr.render(VehicleState(egr_commanded=30.0, egr_error=-4.5), EMPTY)

        assert "(4.5% below commanded)" in text

    def test_explains_a_valve_above_the_command(self) -> None:
        assert "(2.0% above commanded)" in egr.render(VehicleState(egr_error=2.0), EMPTY)

    def test_explains_a_valve_on_target(self) -> None:
        assert "(on target)" in egr.render(VehicleState(egr_error=0.0), EMPTY)

    def test_reports_when_nothing_was_read(self) -> None:
        assert egr.render(VehicleState(), EMPTY) == NO_DATA
...
class TestDiagnostics:
    ...
    def test_shows_the_counters_section(self) -> None:
        text = diag.render(VehicleState(distance_with_mil=120.0), EMPTY)

        assert "COUNTERS" in text
        assert "120" in text

    def test_shows_the_calibration_section(self) -> None:
        assert "CALIBRATION" in diag.render(VehicleState(calibration_id="ABC123"), EMPTY)

    def test_reports_when_nothing_was_read(self) -> None:
        assert diag.render(VehicleState(), EMPTY) == NO_DATA
...
class TestFaults:
    def test_lists_stored_codes_with_their_description(self) -> None:
        ...
        text = faults.render(state, EMPTY)
    ...
    def test_lists_a_code_without_a_description(self) -> None:
        text = faults.render(VehicleState(stored_codes=[TroubleCode("P0401")]), EMPTY)
    ...
    def test_says_so_when_there_is_no_fault(self) -> None:
        assert faults.render(VehicleState(), EMPTY) == faults.NO_FAULTS
...
class TestCatalog:
    ...
    def test_counts_the_supported_commands(self) -> None:
        text = catalog.render(VehicleState(), self._catalog())

        assert "Supported: 1 / 2" in text
        assert "Mode 01 — Live data  (1 / 2)" in text

    def test_marks_each_command(self) -> None:
        lines = catalog.render(VehicleState(), self._catalog()).splitlines()
    ...
    def test_asks_for_a_connection_before_discovery(self) -> None:
        assert catalog.render(VehicleState(), EMPTY) == catalog.NOT_DISCOVERED
